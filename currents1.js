...
(function() {
    ...
    function updateArrows() {
        article.select('.current').each(function(current) {
            ...
            if (index.scrollWidth - index.clientWidth - index.scrollLeft > 120) dom(current).addClass('overflow-right');
            else dom(current).removeClass('overflow-right');

            if (index.scrollLeft > 0) dom(current).addClass('overflow-left');
            else dom(current).removeClass('overflow-left');
        });
    };
    ...
    function currentsScroller(e) {
        ...
        updateArrows();
        if (!e.deltaY || (e.deltaX / e.deltaY) > .8) e.preventDefault();
    };
    ...
    function currentsPager(e) {
        ...
        tween = animate({
            ...
            tick: function(time) {
                index.scrollLeft = Math.round(ease(time));
            },
            done: updateArrows,
        });
